...
# =========================================================================
# Fix: 手動定義 Pure PyTorch 的 selective_scan_ref，解決 libcudart 缺失問題
# =========================================================================

# =========================================================================
# Fix: 強壯版 selective_scan_ref，支援 VMamba 的 Group Broadcasting
# =========================================================================

def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False, return_last_state=False):
    """
    Pure PyTorch implementation of Mamba's selective scan.
    Handles VMamba's specific (B, K, N, L) broadcasting automatically.
    """
    # 轉換型別
    dtype_in = u.dtype
    u = u.float()
    ...
    # [Critical Fix for VMamba]
    # 如果 B 是 4 維 (Batch, K, dstate, L)，代表它是 Group Parameter
    # 我們需要將其擴展(Repeat)以匹配 u 的 dim (dim = K * D_actual)
    if B.dim() == 4:
        # B shape: (Batch, K, dstate, L)
        K = B.shape[1]
        assert dim % K == 0, f"Dimension mismatch: u.dim {dim} not divisible by K {K}"
        D_actual = dim // K
        
        # 廣播操作: (Batch, K, N, L) -> (Batch, K, 1, N, L) -> (Batch, K, D_actual, N, L) -> (Batch, Dim, N, L)
        B = B.view(batch, K, 1, dstate, L).expand(-1, -1, D_actual, -1, -1).reshape(batch, dim, dstate, L)
        
    # 同樣處理 C
    if C.dim() == 4:
        K = C.shape[1]
        D_actual = dim // K
        C = C.view(batch, K, 1, dstate, L).expand(-1, -1, D_actual, -1, -1).reshape(batch, dim, dstate, L)

    # --- 以下為標準 Scan 邏輯 ---

    # A 離散化: deltaA = exp(delta * A)
    # delta: (B, D, L), A: (D, N) -> deltaA: (B, D, L, N)
    deltaA = torch.exp(torch.einsum('bdl,dn->bdln', delta, A))
    
    # B 離散化: deltaB = delta * B
    # delta: (B, D, L), B: (B, D, N, L), u: (B, D, L)
    # deltaB_u = delta * B * u -> (B, D, N, L)
    # 注意: 這裡我們直接用 element-wise 乘法再廣播，比 einsum 更直觀且相容性更好
    # delta unsqueeze -> (B, D, 1, L)
    # u unsqueeze -> (B, D, 1, L)
    deltaB_u = delta.unsqueeze(2) * B * u.unsqueeze(2)

    # Scan (Recurrence)
    x = torch.zeros((batch, dim, dstate), device=u.device)
    ys = []
    
    # 這裡的迴圈是瓶頸，但在沒有 CUDA kernel 的情況下是必須的
    # deltaA: (B, D, L, N) -> permute to (B, D, N, L) for easier indexing
    deltaA = deltaA.permute(0, 1, 3, 2) 
    ...
    for i in range(L):
        ...
        # y = x * C
        if C.dim() == 3: # (B, N, L) - 標準 Mamba 變體
             y = torch.einsum('bdn,bn->bd', x, C[..., i])
        else: # (B, D, N, L) - VMamba 變體
             # x: (B, D, N), C[..., i]: (B, D, N) -> sum over N -> (B, D)
             y = (x * C[..., i]).sum(dim=2)
...
try:
    ...
    print("[INFO] Using optimized CUDA selective_scan_fn from mamba_ssm")
except Exception:
    pass # 失敗也沒關係，我們有上面的 ref 函數

# 這裡很重要：把我們手寫的函數指派給 selective_scan_ref
# 這樣 forward_corev0 裡面調用 self.selective_scan 時就會用到它
try:
    from mamba_ssm.ops.selective_scan_interface import selective_scan_ref as _ref
    # 如果 import 成功，優先用官方的 ref (可能有些微差異但通常沒差)
    # 但如果 mamba_ssm 整個壞掉 (libcudart error)，這裡會失敗，就會用我們定義的函數
except Exception:
    pass
...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        if selective_scan_fn is not None:
            ...
            self.selective_scan = selective_scan_ref  # 慢但可跑
        elif selective_scan_ref_v1 is not None:
            self.selective_scan = selective_scan_ref_v1  # 慢但可跑
        else:
            raise RuntimeError(
                "No selective_scan available. Install mamba_ssm or selective_scan extension."
            )
